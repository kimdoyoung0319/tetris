...
Screen::Screen() {
    ...
    init_pair(BLACK_AND_YELLOW, COLOR_YELLOW, COLOR_BLACK);
...
void Screen::update_main_window(const Board & board) const {
    ...
    for(int i = 0; i < LENGTH_COL; i++) {
        for(int j = 0; j < LENGTH_ROW; j++) {
            position_board = {j, i};
            if(board.is_filled_static(position_board))
                draw_square(position_board);
            else if(board.is_filled_dynamic(position_board))
                draw_square_highlighted(position_board);
            else
...
void Screen::draw_square_highlighted(const Coordinate & position_board) const {
    int cursor_position_x = 2 * position_board.x + 1;
    int cursor_position_y = position_board.y + 1;
    mvwaddch(main_window, cursor_position_y, cursor_position_x, ' ' | A_REVERSE | COLOR_PAIR(BLACK_AND_YELLOW));
    mvwaddch(main_window, cursor_position_y, cursor_position_x + 1, ' ' | A_REVERSE | COLOR_PAIR(BLACK_AND_YELLOW));
